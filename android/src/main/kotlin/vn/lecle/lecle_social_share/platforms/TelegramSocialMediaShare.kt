...
object TelegramSocialMediaShare {
    fun shareFileToTelegram(
        context: Activity,
        ...
        filePath?.let {
            val hasPackage = StoreService.isPackageInstalled(context, TELEGRAM_PACKAGE)
            if (!hasPackage) {
                flutterResult.success(false)
                StoreService.openAppOnStore(context, TELEGRAM_PACKAGE)
            } else {
    ...
    fun sendMessageToTelegram(
        ...
        if (!hasPackage) {
            ...
            sendIntent.setPackage(TELEGRAM_PACKAGE)
            try {
    ...
    fun openTelegramDirectMessage(
        ...
        if (!hasPackage) {
            ...
            val viewIntent = Intent(Intent.ACTION_VIEW, Uri.parse("tg://resolve?domain=$username"))
            viewIntent.setPackage(TELEGRAM_PACKAGE)
    ...
    fun openTelegramChannelViaShareLink(
        context: Activity,
        flutterResult: MethodChannel.Result,
        inviteLink: String,
    ) {
        ...
        if (!hasPackage) {
            ...
            val viewIntent = Intent(Intent.ACTION_VIEW, Uri.parse(inviteLink))
            viewIntent.setPackage(TELEGRAM_PACKAGE)
